...
class DDSM_dataset(data.Dataset):
    def __init__(self, root, view_laterality, split, transform):
        ...
        with open(os.path.join(self.root, '{}_density.txt'.format(self.split)), 'r') as f:
    ...
    def __getitem__(self, idx):
        ...
        if self.view_laterality == 'lcc':
            image = Image.open(os.path.join(self.root, 'images', lcc_img_id )).convert('RGB')
        elif self.view_laterality == 'lmlo':
            image = Image.open(os.path.join(self.root, 'images', lmlo_img_id)).convert('RGB')
        elif self.view_laterality == 'rcc':
            image = Image.open(os.path.join(self.root, 'images', rcc_img_id)).convert('RGB')
        elif self.view_laterality == 'rmlo':
            image = Image.open(os.path.join(self.root, 'images', rmlo_img_id)).convert('RGB')
        elif self.view_laterality == 'left':
            cc_img = Image.open(os.path.join(self.root, 'images', lcc_img_id)).convert('RGB')
            mlo_img = Image.open(os.path.join(self.root, 'images', lmlo_img_id)).convert('RGB')
        elif self.view_laterality == 'right':
            cc_img = Image.open(os.path.join(self.root, 'images', rcc_img_id)).convert('RGB')
            mlo_img = Image.open(os.path.join(self.root, 'images', rmlo_img_id)).convert('RGB')
        else:
