...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input1, input2, input3, input4, target) in enumerate(train_loader):
        ...
        target = torch.tensor(tuple(map(int, target))).to(device)
...
def main(cfg):
    if cfg.training.resume is not None:
        ...
        log_dir = os.path.join(os.path.abspath('.'),cfg.training.logs_dir, '{}_{}'.format(cfg.data.dataset_name, cfg.data.task), cfg.training.fusion_type, cfg.data.analysis)
        checkpoint_dir = os.path.join(os.path.abspath('.'),cfg.training.checkpoints_dir, '{}_{}'.format(cfg.data.dataset_name, cfg.data.task), cfg.training.fusion_type, cfg.data.analysis)
